...
public class SoulCookingPotScreen extends AbstractContainerScreen<SoulCookingPotMenu> implements RecipeUpdateListener {
    private static final String modid = BossesDelight.MODID;
    private static final ResourceLocation RECIPE_BUTTON_LOCATION = new ResourceLocation("textures/gui/recipe_button.png");
    private static final ResourceLocation BACKGROUND_TEXTURE = new ResourceLocation(modid, "textures/gui/soul_cooking_pot.png");
    ...
    private void renderHeatIndicatorTooltip(GuiGraphics gui, int mouseX, int mouseY) {
        if (this.isHovering(HEAT_ICON.x, HEAT_ICON.y, HEAT_ICON.width, HEAT_ICON.height, (double)mouseX, (double)mouseY)) {
            String key = "container.cooking_pot." + (((SoulCookingPotMenu)this.menu).isHeated() ? "heated" : "not_heated");
            gui.renderTooltip(this.font, Component.translatable(modid + "." + key), mouseX, mouseY);
        }

    }

    protected void renderMealDisplayTooltip(GuiGraphics gui, int mouseX, int mouseY) {
        if (this.minecraft != null && this.minecraft.player != null && ((SoulCookingPotMenu)this.menu).getCarried().isEmpty() && this.hoveredSlot != null && this.hoveredSlot.hasItem()) {
            if (this.hoveredSlot.index == 6) {
                ...
                ItemStack containerStack = ((SoulCookingPotMenu)this.menu).blockEntity.getContainer();
                String container = !containerStack.isEmpty() ? containerStack.getItem().getDescription().getString() : "";
                tooltip.add(Component.translatable(modid + "." + "container.cooking_pot.served_on", new Object[]{container}).withStyle(ChatFormatting.GRAY));
